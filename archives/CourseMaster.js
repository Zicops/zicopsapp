...
import { queryClient, GET_CATS_N_SUB_CATS, GET_COURSE } from '../../API/Queries';
...
const CourseMaster = () => {
  ...
  const editCourseId = courseId.courseId;

  const cq = GetCourseDetails(editCourseId);

  // const {cq} =  useQuery(GET_COURSE, {
  //                       variables: {
  ...
  useEffect(() => {
    if (typeof cq != 'undefined') {
      // const { created_at, updated_at, ...getCourseData } = cq.getCourse;
      updateCourseMaster(cq.getCourse);
      console.log(cq);
    }
  }, [cq]);

  // if(editcourseId){
  //     const {getCourse} = GetCourseList(editcourseId);
  //     console.log(getCourse);
  // }

  // useEffect(()=>{
  //   // const { __typename, getCourse1} = getCourse.getCourse;
  //   console.log(getCourse);
  // }, [editcourseId])
    ...
    <div className="course_master">
      <form>
        ...
        <ApolloProvider client={queryClient}>
          <div className="row my_30">
            <label htmlFor="category" className="col_25">
              Course Category
            </label>
            <CreateCatsDropdown inputHandler={inputHandlerSelect} inputField={fullCourse} />
          </div>
          <div className="row">
            <label htmlFor="subcategory" className="col_25">
              Select Base Sub-category
            </label>
        ...
        <div className="row my_30">
          <label htmlFor="owner" className="col_25">
            Course Owner
          </label>
          <select
            ...
            <option className="col_75" value="" hidden>
              Select the owner of the course
            </option>
        ...
        <div className="row my_30">
          <button type="button" className="admin-next-btn" onClick={courseMasterSubmit}>
            Next
          </button>
        </div>
      </form>
