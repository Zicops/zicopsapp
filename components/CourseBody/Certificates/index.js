...
export default function Certificates() {
  ...
  useEffect(() => {
    ...
    setCompletedDate(`${startDate} to ${endDate}`);
  }, [
    userCourseData?.userCourseMapping?.created_at,
    userCourseData?.userCourseMapping?.updated_at
  ]);
  ...
  return (
    <>
      <div className={`${styles.certificate}`}>
        <ZicopsCourseCertificate
          to={`${userData?.first_name || ''} ${userData?.last_name || ''}`}
          courseName={fullCourse?.name}
          // completionDate={userCourseData?.userCourseMapping?.end_date}
