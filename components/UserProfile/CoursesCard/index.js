...
const LinearProgressWithLabel = ({value}) => {
    return (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box sx={{ width: '85%' }}>
                <LinearProgress sx={{height: '5px', borderRadius: '10px', background: '#000',}} variant="determinate" value={value} />
            </Box>
    ...
    return(
        <>
            <ThemeProvider theme={createTheme({
                ...
                <Box color={'#FFF'} width={'100%'} bgcolor={'#292F2F'} boxShadow={'0px 0px 30px 25px rgba(0, 0, 0, 0.25)'} borderRadius={'12px'} p={2} position={'relative'}>
