...
const CurrentCourses = ({ courseData, handleSubmit = () => {}, isLoading = false }) => {
  ...
  const courseSections = [
    ...
    {
      displayType: 'Courses Added by Admin',
      footerType: 'adminFooter',
      data: assignedCourses,
      isRemove: true,
      handleAssign: handleSubmit,
      buttonText: 'Remove'
    },
  ...
  useEffect(() => {
    ...
    setOnGoingCourses(
      [...courseData.filter((course) => course?.isCourseStarted && !course?.isCourseCompleted)],
      setLoading(false)
    );
    setAssignedCourses(
    ...
    setCohortCourses(
      [
        ...courseData?.filter(
          (course) =>
            course?.added_by?.toLowerCase() === ASSIGNED_COURSES[0] ||
            course?.added_by?.toLowerCase() === ASSIGNED_COURSES[2]
        )
      ],
      setLoading(false)
    );
    ...
    setCohortCourses(
      [
        ...courseData?.filter(
          (course) =>
            course?.added_by?.toLowerCase() === ASSIGNED_COURSES[0] ||
            course?.added_by?.role?.toLowerCase() === ASSIGNED_COURSES[2]
        )
      ],
      setLoading(false)
    );
