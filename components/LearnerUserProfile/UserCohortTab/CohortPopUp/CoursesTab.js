...
export default function CoursesTab() {
  ...
  useEffect(async()=>{
   ...
   if(selectedCohort?.cohrotCourses?.length) return setCourseProgress([...selectedCohort?.cohrotCourses],setLoading(false));
    ...
    for(let i = 0 ; i < _cohortCourses?.length ; i++){
      for(let j = 0 ; j < userCourses?.length ; j++){
        ...
        if(userCourses[j]?.id === _cohortCourses[i]?.CourseId){
          _userCohortCourse.push({...userCourses[j],ExpectedCompletion: _cohortCourses[i]?.ExpectedCompletion });
        }
      }
    }
  ...
  return (
    <div className={`${styles.courseTabContainer}`}>
      ...
      <div className={`${styles.listCardTabContainer}`}>
        ...
        !courseProgress?.length && <strong className={`${styles.fallbackMsg}`}>No Courses Found</strong>
      )}
          {courseProgress?.map((course) => (<CourseLIstCard courseData={course} footerType={'onGoing'} />)) }
        </div>
      </div>
    </div>
  );
